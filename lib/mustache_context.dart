...
class _ObjectReflectorDeclaration {
  ...
  Function get lambda => (val, {MustacheContext nestedContext}) {
    var im = mirror.invoke(declaration.simpleName,
        _createPositionalArguments(val), _createNamedArguments(nestedContext));
    return im is InstanceMirror ? im.reflectee : null;
  };

  _createPositionalArguments(val) {
    var positionalParam = declaration.parameters.firstWhere(
        (p) => !p.isOptional, orElse: () => null);
  ...
  _createNamedArguments(MustacheContext ctx) {
    ...
    var nestedContextParameterExists = declaration.parameters.firstWhere(
        (p) => p.simpleName == new Symbol('nestedContext'), orElse: () => null);
  ...
  bool get isVariableOrGetter => (declaration is VariableMirror) ||
